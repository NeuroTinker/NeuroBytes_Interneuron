...
void sys_tick_handler(void)
{
	...
	*/
	...
	for (i=0; i<NUM_PINS; i++){
		if (all_pins[i]->io_state == INPUT && all_pins[i]->active == true){
			if (read_tick == all_pins[i]->input_buffer->read_tick)
				readBit(all_pins[i]);
		}
	}
...
void pinInit(void)
{
	...
	all_pins[1]->input_buffer = initInputBuffer();
	...
	setPinAsInput(all_pins[1]);
	all_pins[2]->input_buffer = initInputBuffer();
	...
	all_pins[3]->input_buffer = initInputBuffer();
...
void selectOutputPins(pin_group_t pin_group)
{
	uint8_t i;
	switch (pin_group){
		case ALL:
