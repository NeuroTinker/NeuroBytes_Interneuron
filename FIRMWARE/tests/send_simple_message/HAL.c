...
static pin_t * all_pins[NUM_PINS];
static pin_t * active_output_pins;
static uint8_t num_active_output_pins;
...
void sys_tick_handler(void)
{
	...
	pin_group_t output_pins = getOutputPins();
	pin_group_t input_pins = getInputPins();
	...
	/*
		For bit-bang, read_tick = tick % 3.
		Not sure what the most efficient way to keep tick % 3 is
	*/
	read_tick += 1;
	if (read_tick > 2){
		read_tick = 0;
		writeBit(output_pins);
	}

	for (i=0; i<input_pins->num_pins; i++){
		readBit(input_pins->pins[i])
	}
