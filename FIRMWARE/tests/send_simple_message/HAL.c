...
pin_t * all_pins[NUM_PINS];
pin_t * input_pins[NUM_INPUTS];
...
void pinInit(void)
{
	// generated by pin-map config 
	break1;
	// axon 1 excit A8
	all_pins[0]->address->port = PORT_AXON1_EX;
	all_pins[0]->address->pin = PIN_AXON1_EX;
	...
	// axon 2 inhib B0
	all_pins[1]->address->port = PORT_AXON1_IN;
	all_pins[1]->address->pin = PIN_AXON1_IN;
	...
	// dendrite 1 excit B1
	all_pins[2]->address->port = PORT_DEND1_EX;
	all_pins[2]->address->pin = PIN_DEND1_EX;
	...
	all_pins[3]->address->port = PORT_DEND1_IN;
	all_pins[3]->address->pin = PIN_DEND1_IN;
	...
	input_pins[0] = all_pins[0];
	input_pins[1] = all_pins[2];
	input_pins[2] = all_pins[3];
	// initialize bit-bang
	initBitBang();
}
...
void startRead(pin_t * pin, uint8_t bits_to_read)
{
	...
	*/
	...
	pin->active_input->num_bits_to_read = bits_to_read;
}
